...
BufferManager::BufferManager(const int bufs) {
    ...
    bufferhashTable = new BufferHashTable(prime(bufs)); // 创建散列表
}

// 析构函数

BufferManager::~BufferManager() {
    Status s = commit(); // 将所有脏页写回磁盘
    if (s != OK) cout << "注销缓存管理器时无法写出所有脏页！" << endl;
...
const Status BufferManager::commit() {
    ...
    for (int i = 0; i < bufferNumbers; i++) {
        ...
        if (bufInfo->pinCnt > 0) {
            cout << "文件名：" << bufInfo->file->getFileName() << endl;
            cout << "页编号：" << bufInfo->pageNo << endl;
            cout << "摁钉数：" << bufInfo->pinCnt << endl;
            return PAGEPINNED;
        }
        if (!bufInfo->empty && bufInfo->dirty && bufInfo->file) {
...
const Status BufferManager::rejectFile(File* file) {
    ...
    for (int i = 0; i < bufferNumbers; i++) {
        BufferInfo* tmpbuf = &(bufferInfo[i]);
        if (tmpbuf->empty == false && tmpbuf->file == file) {
            if (tmpbuf->pinCnt > 0)
                return PAGEPINNED;
            ...
            bufferhashTable->remove(file, tmpbuf->pageNo);

            bufferInfo[i].Clear();
        } else if (tmpbuf->empty == true && tmpbuf->file == file) {
            return BADBUFFER;
        }
    }
...
const Status BufferManager::chooseFrame(int &frame) {
    ...
    while (true) {
        advanceClock();
        victim = &bufferInfo[clockHand];

        if (victim->pinCnt > 0) pinFrames++;
        else pinFrames--;

        if (pinFrames == bufferNumbers) return BUFFEREXCEEDED; // 全部被钉住
        if (victim->empty == true) {
            break;
        } else {
            if (victim->refbit == true) {
                victim->refbit = false;
                continue;
            }
        }
    ...
    frame = victim->frameNo; // 返回frame
    if (victim->empty == true) // 页帧未使用
    {
        victim->Clear();
        return OK;
    } else {
        ...
        if ((status = bufferhashTable->remove(victim->file, victim->pageNo)) != OK) return status;
        victim->Clear();
    }
    return OK;
}